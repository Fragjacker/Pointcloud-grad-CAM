...
def train( poolingMethod ):
    with tf.Graph().as_default():
        ...
        if not performTraining:
            maxgradients = computeHeatGradient( pred, feature_vec, testLabel, poolingMethod )
        else:
            maxgradients = None
        ...
        sys.stdout.flush()
        heatMap, curPointmaxCloud = test_rotation_XYZ( sess, ops, test_writer )
        sess.close()
        return heatMap, curPointmaxCloud

def eval_perturbations( numInputPoints, perturbedData, mode, poolingMethod ):
    with tf.Graph().as_default():
        with tf.device( '/gpu:' + str( GPU_INDEX ) ):
        ...
        log_string( '**** TESTING MODEL ****' )
        sys.stdout.flush()
        maxgradients = test_perturbed_pc( sess, ops, perturbedData, mode )
        sess.close()
        return maxgradients
...
#         print("LENGTH GRADIENTS: ", len(maxgradients))
...
#
#             plt.plot(np.arange(len(maxgradients)), maxgradients, 'C0', label='Original gradient')
...
#         tdh.writeAllResults(filePath, testResults)
    ...
    storeTestResults( mode, total_correct_PDel, total_seen_PDel, loss_sum_PDel, pred_val )
...
#
#             print("LENGTH GRADIENTS: ", len(maxgradients))
#         plotTwoResults(rotated_data, maxgradients, "average")
#         plotTwoResults(rotated_data, maxgradients, "median")
#         plotTwoResults(rotated_data, maxgradients, "midrange")
#         print("ROTATION PREDICTION: ", getPrediction(pred_val))
#         sys.exit()

#         filePath = getShapeName(testLabel)+"_"+str(numTestBatchSize)+"_"+str(fn)+"_Y_corrected_maxpooled_accuracy"
#         print("STORING FILES TO: ", filePath)
#         tdh.writeAllResults(filePath, accuracyResults)
#         filePath = getShapeName(testLabel)+"_"+str(numTestBatchSize)+"_"+str(fn)+"_Y_corrected_maxpooled_meanloss"
#         print("STORING FILES TO: ", filePath)
#         tdh.writeAllResults(filePath, meanlossResults)
    log_string( 'Original Prediction: ' + getPrediction( pred_val ) )
    log_string( 'Original mean loss: %f' % ( loss_sum / float( total_seen ) ) )
    log_string( 'Original accuracy: %f' % ( total_correct / float( total_seen ) ) )
#     log_string('eval avg class acc: %f' % (np.mean(np.array(total_correct_class)/np.array(total_seen_class,dtype=np.float))))
    return maxgradients, currentPointCloud
...
if __name__ == "__main__":
    ...
    for iteration in range( 20 ):
        ...
        resultPCloudThresh, _ = gch.delete_above_threshold( maxMediangradients, maxCurPointCloudMedianResult, "median" )
        ...
        # Maxpooling
        #------------------------------------------------------------
        resultPCloudThresh, delCount = gch.delete_below_threshold( maxAvggradients, maxCurPointCloudAverageResult, "average" )
        maxCurPointCloudAverageResult = copy.deepcopy( resultPCloudThresh )
...
#     gch.draw_heatcloud(resultPCloudThresh, maxMediangradients, "median")
        # Average pooling
        #------------------------------------------------------------
    ...
    testResultsacc3 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_maxpooled_zeros_removed_accuracy" )
    testResultsacc4 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_average_removed_accuracy" )
    testResultsacc5 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_median_removed_accuracy" )
    testResultsacc6 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_midrange_removed_accuracy" )
    testResultsacc7 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_zeros_removed_accuracy" )
    testResultsacc8 = tdh.readTestFile( "airplane_1000_XYZ_maxpooled_random_removed_accuracy" )
    ...
    testResultsloss3 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_maxpooled_zeros_removed_meanloss" )
    testResultsloss4 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_average_removed_meanloss" )
    testResultsloss5 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_median_removed_meanloss" )
    testResultsloss6 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_midrange_removed_meanloss" )
    testResultsloss7 = tdh.readTestFile( "airplane_1000_XYZ_unimportant_avgpooled_zeros_removed_meanloss" )
    testResultsloss8 = tdh.readTestFile( "airplane_1000_XYZ_maxpooled_random_removed_meanloss" )
    ...
    plt.plot( np.arange( len( testResultsacc3 ) ), testResultsacc3, label = "Maxpooled zeros removed" )
    plt.plot( np.arange( len( testResultsacc4 ) ), testResultsacc4, label = "Averagepooled average removed" )
    plt.plot( np.arange( len( testResultsacc5 ) ), testResultsacc5, label = "Averagepooled median removed" )
    plt.plot( np.arange( len( testResultsacc6 ) ), testResultsacc6, label = "Averagepooled midrange removed" )
    plt.plot( np.arange( len( testResultsacc7 ) ), testResultsacc7, label = "Averagepooled zeros removed" )
    plt.plot( np.arange( len( testResultsacc8 ) ), testResultsacc8, label = "Random removed" )
    ...
    plt.plot( np.arange( len( testResultsloss3 ) ), testResultsloss2, label = "Maxpooled zeros removed" )
    plt.plot( np.arange( len( testResultsloss4 ) ), testResultsloss3, label = "Averagepooled average removed" )
    plt.plot( np.arange( len( testResultsloss5 ) ), testResultsloss4, label = "Averagepooled median removed" )
    plt.plot( np.arange( len( testResultsloss6 ) ), testResultsloss5, label = "Averagepooled midrange removed" )
    plt.plot( np.arange( len( testResultsloss7 ) ), testResultsloss6, label = "Averagepooled zeros removed" )
    plt.plot( np.arange( len( testResultsloss8 ) ), testResultsloss7, label = "Random removed" )
    plt.legend( title = ( "Point removal plot" ) )
    plt.ylabel( "Loss" )
    plt.xlabel( "Iterations" )
    plt.show()
